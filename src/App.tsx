...
import type { ChatMessage, Persona, PurchaseRequisition } from "./types";
...
function App() {
  ...
  const addMessage = (role: "user" | "assistant" | "system", content: string, prNumber?: string) => {
  ...
  const handleSendMessage = (userMessage: string) => {
    ...
    if (parsed.action === "create") {
      // Create a new PR
      try {
        const pr = createPurchaseRequisition(userMessage, selectedPersona);
        setSelectedPR(pr);
        ...
        addMessage(
          "assistant",
          `Sorry, I encountered an error creating the PR: ${error}`
        );
      }
    } else if (parsed.action === "status") {
      // Look up a specific PR
      if (parsed.prNumber) {
      ...
      if (allPRs.length === 0) {
        ...
        let response = `**Your Purchase Requests** (${allPRs.length} total):\n\n`;
        allPRs.slice(0, 10).forEach((pr) => {
          response += `• **${pr.prNumber}** - ${pr.contextInference.category} (${pr.status})`;
          if (pr.intentClassification.needsHumanReview) {
            response += " ⚠️";
          }
          response += `\n  ${pr.lineItems[0]?.description.substring(0, 60)}${pr.lineItems[0]?.description.length > 60 ? "..." : ""}\n`;
        });

        if (allPRs.length > 10) {
          response += `\n_Showing 10 of ${allPRs.length} PRs_`;
        }
      ...
      <header className="border-b bg-card px-6 py-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-bold">PR→PO Intake Agent</h1>
          </div>
          <PersonaSelector
